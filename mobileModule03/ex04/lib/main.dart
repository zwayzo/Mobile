...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget _buildLocationHeader() {
    ...
    return Padding(
      padding: EdgeInsets.only(
        top:    scale(context, isLandscape ? 3 : 4),  // ← was 6/12, now much smaller
        bottom: scale(context, isLandscape ? 3 : 30),  // ← keep a little space below
      ),
      child: Text(
  ...
  Widget buildTodayTab() {
    ...
    final List<double> chartTemps = temps.take(24).map((e) => (e as num).toDouble()).toList();
    ...
    return Column(
      children: [
        ...
        Padding(padding: EdgeInsets.symmetric(vertical: scale(context, 16)),
        child: buildTemperatureChart(chartTemps),
        )
      ],
  ...
  Widget buildTemperatureChart(List<double> temps) {
  return SizedBox(
    height: 200,
    child: LineChart(
      LineChartData(
        ...
        titlesData: FlTitlesData(
          ...
          leftTitles: AxisTitles(
            sideTitles: SideTitles(
              showTitles: true,
              reservedSize: 36,        // ← space reserved to the left of the chart
              getTitlesWidget: (value, meta) {
                return SideTitleWidget(
                  axisSide: meta.axisSide,
                  child: Text(
        ...
        lineBarsData: [
          LineChartBarData(
            ...
            belowBarData: BarAreaData(show: true, color: const Color.fromARGB(255, 255, 2, 0).withOpacity(0.3),),
            color: const Color.fromARGB(255, 255, 2, 0).withOpacity(0.3),
          ),
        ],
      ),
    ),
  ...
  // Sky palette — matches the daily chart
  const Color minColor = Color(0xFF8AB4D4); // kTextSecondary — cool steel blue for min
  const Color maxColor = Color(0xFFD6E8F5); // kTextPrimary  — pale sky for max
      ...
      LineChartData(
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (value, meta) {
                ...
                final day = DateTime.parse(dates[index]);
                return SideTitleWidget(
                  axisSide: meta.axisSide,
                  child: Text(
        ...
        lineTouchData: LineTouchData(
          ...
          touchTooltipData: LineTouchTooltipData(
            getTooltipColor: (_) => const Color.fromARGB(255, 255, 255, 255), 
            tooltipRoundedRadius: 12,
            getTooltipItems: (spots) {
              return spots.map((s) {
                final isMax = s.barIndex == 1;
                return LineTooltipItem(
                  ...
                  children: [
                    TextSpan(
                      text: isMax ? " max" : " min",
                      style: TextStyle(
                        color: const Color.fromARGB(255, 224, 170, 170),
        ...
        lineBarsData: [
          ...
          LineChartBarData(
            spots: minSpots,
            isCurved: true,
            color: const Color.fromARGB(255, 9, 44, 221),
            barWidth: 3,
            dotData: FlDotData(show: false),
            belowBarData: BarAreaData(
              show: false,
              color: const Color.fromARGB(255, 9, 44, 221).withOpacity(0.08),
            ),
          ),
          ...
          LineChartBarData(
            ...
            belowBarData: BarAreaData(
              show: false,
              // color: const Color.fromARGB(255, 255, 2, 0).withOpacity(0.08),
            ),
          ),
        ],
      ),
  ...
  Widget buildWeeklyTab() {
    ...
    for (int i = 0; i < dates.length; i++) {
      minSpots.add(FlSpot(i.toDouble(), (minTemps[i] as num).toDouble()));
      maxSpots.add(FlSpot(i.toDouble(), (maxTemps[i] as num).toDouble()));
    }
    ...
    return Column(
      children: [
        ...
        Padding(padding: EdgeInsets.symmetric(vertical: scale(context, 16)),
          child: buildWeeklyChart(minTemps, maxTemps, dates, maxSpots, minSpots),
        ),
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          SafeArea(
            child: Padding(
              padding: EdgeInsets.only(top: kToolbarHeight -scale(context, 50)),
              child: Column(
                children: [
